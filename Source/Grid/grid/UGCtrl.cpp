...
****************************************************/
BOOL CUGCtrl::AttachGrid(CWnd * wnd,UINT ID,int cols,int rows,int grabcnt){
	...
	if( SubclassDlgItem(ID,wnd))
	{
		...
		OnSetup();
		//SetGrid_LED( cols, rows);
		SetGrid_LED(cols, rows, grabcnt);
		OnSheetSetup(0);
...
int CUGCtrl::RedrawAll(){
	...
	//nht
	m_cell.SetBorder(UG_BDR_LTHICK);
	return UG_SUCCESS;
}
/***************************************************
RedrawCell
...
void CUGCtrl::SetGrid_LED(int cols,int rows,int grabcnt){
}

void CUGCtrl::SetGridOkNg_LED(BOOL OkNg,int col,int row){
}
/****************************************************
OnReset
